...
<p>Each UTXO has an ID, which is generated by hashing its owner's name and public key, along with its value and salt, and a value. It also has an attribute 'used', which "reserves" the UTXO for a transaction, so it is not used in other transactions after already being used in one. It also has a change flag to determine if this UTXO was given to a user as change in a previous transaction, but this is purely to simplify reassigning outputs after mining and writing transaction logs and does not affect the calculations whatsoever.</p>
<h2>Merkle Root hash implementation</h2>
<p>Rather than using a Merkle tree coded using regular C++ data types, v0.3 implements libbitcoin data types for the Merkle tree. In order for this to work, the transaction list that was passed into the previous Merkle tree had to instead be converted to a hash list of their IDs, and the block structure that previously stored the Merkle Root hash as a string had to be converted to hash digest data type.
...
<h2>Screenshots</h2>
<img width="1119" height="797" alt="Screenshot 2025-11-06 004749" src="https://github.com/user-attachments/assets/c2c4969b-2c0a-425b-8432-8f738938865e" />
<img width="903" height="723" alt="Screenshot 2025-11-06 004708" src="https://github.com/user-attachments/assets/fdbf5b55-cad6-43ae-83d9-620e924bc852" />
<img width="988" height="849" alt="Screenshot 2025-11-06 004440" src="https://github.com/user-attachments/assets/bc10af50-6edd-4e85-b698-ebdfd151dd49" />
<h2>Setup</h2>
